...
func (r *Repository) SimilaritySearch(ctx context.Context, userID string, embedding []float32, limit int, filters SearchFilters) ([]SearchResult, error) {
	...
	`
	args := []interface{}{vecStr, workspaceID}
	argIdx := 3
...
func (r *Repository) SaveChatMessage(ctx context.Context, msg ChatMessage) error {
	...
	_, err := db.DB.ExecContext(ctx, `
		INSERT INTO ai_chat_messages (
			id, workspace_id, session_id, meeting_id, role, content, citations, input_tokens, output_tokens, model
		) VALUES (
			gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8, $9
		)
	`, msg.WorkspaceID, msg.SessionID, msg.MeetingID, msg.Role, msg.Content, string(citationsJSON), msg.InputTokens, msg.OutputTokens, msg.Model)
...
func (r *Repository) GetChatHistory(ctx context.Context, sessionID, userID string, limit int) ([]ChatMessage, error) {
	...
	rows, err := db.DB.QueryContext(ctx, `
		SELECT id, workspace_id, session_id, meeting_id, role, content, citations, input_tokens, output_tokens, model, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&m.ID, &m.WorkspaceID, &m.SessionID, &meetingID, &m.Role, &m.Content, &citationsJSON,
